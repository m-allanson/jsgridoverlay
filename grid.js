// basic namespace, wrap everything in self exec anon func
(function(){ 
	// todo: use strict?

	// Star with micro templating taken from underscore.js
	var _ = {};
	...
	// based on the underscore.js extend()/each(), but only uses native foreach.
	// see http://documentcloud.github.com/underscore/docs/underscore.html#section-67
	_.extend = function(obj) {
		// Get any extra passed in arguments as an array.
	...
	// an inArray implementation. ex-jQuery? not in jQuery any more.
	...
	// see https://github.com/jimmydo/js-toolbox/blob/master/toolbox.js
	// Usually uses underscore's extend method, of which there is a custom version above.
	(function () {
	    "use strict";
	...
	var Model = Toolbox.Base.extend({
		...
		_load: function () {
			...
			for (var attr in attrs) {
				result = localStorage.getItem(prefix + attr);
				if (result && (result !== null)) { // TODO: check the best way to test for a result
					attrs[attr] = result;
				}
			}
		},
		...
		// var newAttrs is {key: value, key2: value... }
		set: function (newAttrs) {
			console.log('Model.set', newAttrs);
			var attrs = this.attributes;

			attrs = _.extend(attrs, newAttrs); // set newAttrs to attrs
...
console.log(event.which);
	...
	var View = Toolbox.Base.extend({
		constructor: function (template, model) {
			...
			if (parseInt(this._model.attributes.isActive, 10)) { this.attach(); }
		},
	...
	var OverlayView = View.extend({
		...
		setBackgroundImage: function() {
			...
			if (bg !== 'none') {
				...
				// ditch the bg color
				myModel.set( {'backgroundColor': 'transparent'} );  // TODO: no explicit model reference please
				this.update();
			}
		}
	});
	...
	// now we can define some vars and fire everything up
